...
from tmb_app.common import connect
...
def get_message(template_name, endpoint):
    if request.method == 'GET':
        # Handle GET request: display messages
        cursor = getCursor()
        cursor.execute('SELECT message_id, title, content, created_at, messages.user_id, CONCAT(firstName, " ", lastName) AS fullname FROM messages JOIN users ON messages.user_id = users.user_id ORDER BY created_at DESC')
        ...
        return render_template(template_name, username=session['username'], user_role=session['role'], user_message_info=user_message_info)
        ...
        # return render_template(template_name, username=session['username'], user_role=session['role'])




def get_thread(message_id, endpoint):
    user_id = session['id']
    if request.method == 'POST':
        ...
        if content:
            ...
            # cursor.commit()
        return redirect(url_for(endpoint, message_id=message_id))
        ...
        return render_template('thread.html', thread_detail = thread_detail, message_id=message_id)



def get_delete_message (message_id, endpoint):   #如果 msg_id 不是表单数据，但你仍然想通过 URL 参数传递它，你可以将模板中的删除按钮结构化，以将 msg_id 作为 URL 的一部分。
    cursor = getCursor()
...
def get_delete_reply (reply_id, endpoint):
    cursor = getCursor()
